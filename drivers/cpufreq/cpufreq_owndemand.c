/*
 *  drivers/cpufreq/cpufreq_owndemand.c
 *
...
bool owndemand_is_active = false;
...
/* cpufreq_owndemand Governor Tunables */
...
void owndemand_screen_is_on(bool state)
{
...
void owndemand_boostpulse(bool boost_for_button)
{
	if (!boostpulse_relayf)
	{
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_OWNDEMAND
static
#endif
struct cpufreq_governor cpufreq_gov_owndemand = {
	.name			= "owndemand",
...
{
	dbs_wq = alloc_workqueue("owndemand_dbs_wq", WQ_HIGHPRI | WQ_UNBOUND, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create owndemand_dbs_wq workqueue\n");
		return -EFAULT;
	}
	...
	return cpufreq_register_governor(&cpufreq_gov_owndemand);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_owndemand);
	destroy_workqueue(dbs_wq);
}

MODULE_AUTHOR("Alexander Clouter <alex@digriz.org.uk>");
MODULE_DESCRIPTION("'cpufreq_owndemand' - A dynamic cpufreq governor for "
		"Low Latency Frequency Transition capable processors "
		"optimised for use in a battery environment");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_OWNDEMAND
